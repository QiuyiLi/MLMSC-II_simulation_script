...
class HaplotypeTree:
    ...
    def Dprocess(self, unlinked, distanceAboveRoot, threshold=float('inf'), event=None):
        ...
        # trivial case
        if len(self.getNodes()) == 1:
            # print(distanceAboveRoot)
            # print(event['eventHeight'])
            # print('='*40)
            distanceAboveRoot = event['eventHeight']
            ...
            if speciesId == None:
                ...
                while speciesNode.parent >= 0:
                    if eventHeight >= self.speciesTree.getDistanceToLeaf(speciesNode.parent):
                        ...
                        break
                # if speciesId != original_speciesId:
                #     print(geneName)
                #     print(speciesNode)
                #     print('='*40)
        # if speciesId == None:
    ...
    def DTLprocess(self, locusTree, haplotypeTree, initial=False, event=None, rootLength=0):
        ...
        # pprint.pprint(haplotypeTreeEvents)
        # pprint.pprint(newCoalescentTreeEvents)
        # pprint.pprint(newEvents)
        # print('='*40)
        return newEvents
            ...
            if not passed:
                ...
                if distanceAboveSpeciesNode < 0:
                    ...
                    if coalDistance <  min(abs(distanceAboveSpeciesNode), lossDistance):
                        ...
                        if branchLength + event['eventHeight'] > self.locusTree.getTreeHeight() + self.locusTree.distanceAboveRoot:
                            # exceed the locus tree height
                            # print('discard0')
                            # print(self.locusTree)
                        ...
                        # print('unselected tree0')
                        # print(geneNodeName)
                        # print(nodeId)
                        # pprint.pprint(selectedCoalescentProcess)
                        # pprint.pprint(fullCoalescentProcess)
                        return None, None, None
                    elif lossDistance < min(abs(distanceAboveSpeciesNode), coalDistance):
                        # print('get lost0')
                        # print(lossDistance, distanceAboveSpeciesNode, coalDistance)
